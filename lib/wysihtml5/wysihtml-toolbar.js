...
;(function(wysihtml5){
    var NODE_NAME = 'x-revision';
    var XFoo = document.registerElement(NODE_NAME, {
        prototype: Object.create(HTMLButtonElement.prototype),
        extends: 'span'
    });
    
    wysihtml5.commands.revision = {
        exec: function(composer, command, value) {
            value = typeof(value) === "object" ? value : { notes: value };

            var revision   = this.state(composer),
                    doc     = composer.doc;
            ...
            if (revision) {
                for (var key in value) {
                    if (value.hasOwnProperty(key)) {
                        revision.setAttribute(key === "className" ? "class" : key, value[key]);
                    }
                }
                return;
            }
            ...
            revision = doc.createElement(NODE_NAME);

            for (var i in value) {
                revision.setAttribute(i === "className" ? "class" : i, value[i]);
            }

            composer.selection.insertNode(revision);
            //wysihtml5.commands.formatInline.execWithToggle(composer, command, "revision", "wysiwyg-color-revision");
